...
export async function POST(request: NextRequest) {
  ...
  const { roomId, playerId, source } = await request.json()
  ...
  // Update room (but DON'T add card to hand yet - it stays in limbo)
  await supabase
    .from('game_rooms')
    .update({
      deck: newDeck,
  ...
  await supabase.from('game_actions').insert({
    ...
    player_id: playerId,
    action_type: 'draw',
    action_data: { source, cardId: drawnCard.id },
    result: 'success'
  })
  
  // Return the card ONLY to the player who drew it (don't store in DB)
